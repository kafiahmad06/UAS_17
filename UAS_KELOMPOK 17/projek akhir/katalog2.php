...
<nav class="bg-gray-300 shadow-lg sticky top-0 z-50">
  <div class="max-w-7xl mx-auto px-4 py-4 flex justify-between items-center">
    ...
    <div id="nav-menu" class="hidden md:flex space-x-6 items-center font-bold">
      <a href="halaman1.php" class="hover:text-blue-500">Home</a>
      <a href="#katalog" class="hover:text-blue-500">Katalog</a>
...
<section id="katalog" class="py-16 px-6 md:px-20">
  ...
  <div class="flex overflow-x-auto space-x-4 mb-10 pb-2">
    <?php
    ...
    while ($r = mysqli_fetch_assoc($rumah)) {
      ...
      echo '
      <div class="min-w-[250px] bg-white rounded-lg shadow-md overflow-hidden hover:scale-95 transition-all duration-300">
        <a href="detailproperty2.php?tipe=' . urlencode($r['tipe']) . '&lokasi=' . urlencode($r['lokasi']) . '&status=' . urlencode($r['status']) . '&harga=' . urlencode($r['harga']) . '&gambar=' . urlencode($r['gambar']) . '">
  ...
  <div class="flex overflow-x-auto space-x-4 pb-2">
    ...
    while ($a = mysqli_fetch_assoc($apartemen)) {
      ...
      echo '
      <div class="min-w-[250px] bg-white rounded-lg shadow-md overflow-hidden hover:scale-95 transition-all duration-300">
        <a href="detailproperty2.php?tipe=Tower ' . urlencode($a['tipe']) . '&lokasi=' . urlencode($a['lokasi']) . '&status=' . urlencode($a['status']) . '&harga=' . urlencode($a['harga']) . '&gambar=' . urlencode($a['gambar']) . '">
